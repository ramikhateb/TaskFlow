...
- FR-28 **(M9)** A user can view their Inbox: `PENDING` assignments sent to them, newest first. No status filter is implemented — Inbox is deliberately PENDING-only (an actionable queue, not a history view; see FR-29 for the historical counterpart).
- FR-29 **(M10)** A user can view their Sent list: every assignment they've sent, in any status (`PENDING`/`ACCEPTED`/`DECLINED`/`CANCELLED`), newest first — this is the historical counterpart to Inbox's PENDING-only queue. No status filter is implemented (all statuses are always returned together); a client wanting to narrow the view does so locally.
...
- FR-32 **(M10)** The original creator retains permanent read access to the task after assignment, but not write access, unless they are also the current assignee. "Read access" here is deliberately narrower than the current assignee's own view: `scheduledAt` — the current assignee's personal planning state — is never exposed to a non-assignee creator (the server always sends `null` for it to that viewer, regardless of the task's real stored value); title, description, priority, category, deadline, status, and `completedAt` are all visible, since they describe the task/work itself rather than the assignee's personal schedule. `GET /tasks/:id`'s response additively includes a `viewer: { isAssignee, isCreator, canEdit, canDelete }` object so the client reflects this server-computed distinction instead of inferring it (see [ARCHITECTURE.md](./ARCHITECTURE.md) §3). This is a **read-only** grant: `creatorId` is never a substitute for `assigneeId` on `PATCH`/`DELETE`, which remain current-assignee-only exactly as before M10. It does not extend to every past sender/recipient in a task's assignment history — only the *original* creator and the *current* assignee ever have standing access to the live task (see EC-16).
...
- **EC-16 Historical participation is not permanent access (M10).** Having been a task's creator, a past assignee, or a past assignment's sender/recipient does *not*, by itself, grant standing read access to the *current* task — only the *original* creator (`Task.creatorId`, which never changes) and the *current* assignee (`Task.assigneeId`) do (FR-32). In a transfer chain (Rami creates → Daniel accepts → Daniel reassigns to Sarah, who accepts), Daniel — no longer creator or current assignee — gets a plain 404 from `GET /tasks/:id`, the same enumeration-safe response an unrelated stranger would get, even though he's genuinely in that task's `TaskAssignment` history and can still see his own sent request in his own Sent list (FR-29). "I have a `TaskAssignment` row referencing this task" and "I may read this task" are deliberately two different questions with two different answers.
- **EC-17 Assignment history and task deletion (v1 limitation).** `TaskAssignment.task`'s `onDelete: Cascade` (M8, EC-7) means deleting a task removes its entire assignment history with it — including rows a *different* user's Sent list would otherwise still show. This was an acceptable, deliberate v1 tradeoff when EC-7 first shipped (deletion is only reachable with no `PENDING` assignment in the way), and M10's Sent view doesn't change that: a sender's Sent entry for a task the current assignee later deletes simply disappears too, rather than persisting as a stand-alone historical record. Preserving Sent history past task deletion would need snapshotting or soft-deletion, which is a real schema change, not something this milestone does — see [DATABASE.md](./DATABASE.md) §8 for the full discussion and why it's deferred rather than fixed speculatively.
