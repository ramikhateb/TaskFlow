...
- **Features (`src/features/<feature>/`)** — feature-scoped hooks, components, and API-call wrappers: `src/features/tasks`, `src/features/users`, `src/features/inbox` (M9, now also Sent — M10). TanStack Query hooks live here (`useTodayTasks`, `useCreateTask`, `useUserSearch`, `useInbox`, `useAcceptAssignment`, `useDeclineAssignment`, `useSentAssignments`, etc.). **(M8)** Assignment create/cancel hooks (`useCreateAssignment`, `useCancelAssignment`) stayed in `src/features/tasks/useAssignments.ts` rather than a standalone folder, since M8 had no assignment-specific screen yet — the only UI surface was the task detail screen. **(M9)** Now that the Inbox exists as its own screen with its own list/accept/decline concerns, that surface gets its own `src/features/inbox/` folder; the M8 create/cancel hooks stay where they are (still task-detail-scoped, not Inbox-scoped) rather than being merged in, since they serve a different screen and a different caller (the sender, not the recipient). **(M10)** Sent's own hooks (`useSentAssignments`) live in that same `src/features/inbox/` folder — Sent is a second *view* of the same Inbox screen (see Navigation below), not a separate screen, so it shares the feature folder rather than getting its own; `useCancelAssignment` (M8, still in `features/tasks/`) is reused as-is from a Sent row, and both `useCreateAssignment`/`useCancelAssignment` now additionally invalidate Sent's query key, imported from `features/inbox/useInbox.ts` — the same cross-feature-folder invalidation pattern `useAcceptAssignment` already established in the other direction (importing `TASKS_QUERY_KEY` from `features/tasks/`).
- **Navigation (Expo Router)** — `app/(app)/_layout.tsx`'s `Tabs` gained a fifth visible destination, `inbox`, alongside Today/Schedule/Tasks/Profile (M9). Its detail/response screen follows the exact pattern already established for Tasks: a sibling folder (`app/(app)/inbox-detail/`) holding a `Stack`-wrapped `[id].tsx`, hidden from the tab bar via `href: null` and reached with `router.push(...)` — never a second `Tabs.Screen`. This was a deliberate reuse of the existing convention rather than a new navigation pattern. **(M10)** Sent did **not** get a sixth tab or its own route: the single `inbox` screen gained an in-screen Incoming/Sent toggle (local `useState`, not a route), keeping the five-tab bar (Today/Schedule/Tasks/Inbox/Profile) exactly as it was — collaboration (incoming and outgoing) stays one destination with two views, per the explicit M10 instruction not to crowd the tab bar. Tapping a Sent row navigates straight to the existing `/tasks/:id` screen (no separate "Sent detail" screen at all) — that screen's own M10 `viewer`-driven rendering already produces the right result whether the sender is still the assignee or is now creator-only, so a second detail screen would just be a redundant, un-server-driven copy of that same logic.
...
| `GET /tasks/:id` | Task detail — readable by the current assignee **or** the original creator (M10, FR-32); additively includes `pendingAssignment` (M8), `assignee`/`creator` (`PublicUser`) and `viewer` capabilities (M10, see "Creator-safe task-detail contract" below) |
| `PATCH /tasks/:id` | Update fields, including `status` (current assignee only, unchanged by M10's read-access widening; rejected with 409 while a `PENDING` assignment exists — FR-13/EC-5, enforced M8) |
| `DELETE /tasks/:id` | Delete (current assignee only, unchanged by M10; rejected with 409 while a `PENDING` assignment exists — FR-13/EC-5, enforced M8) |
...
**Assignments** — create/cancel are nested under `/tasks/:taskId` since they're scoped to one task and its current assignee (the sender), keeping the authorization check ("is the caller this task's current assignee?") symmetric with the rest of the `/tasks` resource. Inbox/accept/decline (M9) are addressed by assignment id alone and scoped to the *recipient*, not the task's assignee, so they're mounted flat at `/assignments` instead (`routes/inboxRoutes.ts`) — nesting them under a task id would add nothing, since a recipient browsing their Inbox isn't necessarily thinking in terms of a specific task's id.
...
| `GET /tasks/:id` | *(existing M3 route)* additionally returns `pendingAssignment: TaskAssignmentResponse \| null` (M8) and `assignee`/`creator`/`viewer` (M10) — built by `TaskService.getTaskDetail`, repository-level dependencies, not a call into `AssignmentService` (see "Final task-detail dependency structure" below) | **M8/M10** |
| `GET /assignments/inbox` | Incoming `PENDING` requests for the authenticated caller (`toUserId` from the JWT only — no query param accepted), newest first; each row is an `InboxAssignmentResponse` (sender `PublicUser` + a task summary, deliberately excluding `scheduledAt` — see "Inbox response shape" below) | **M9** |
| `POST /assignments/:assignmentId/accept` | Accept; body `{ scheduledAt: string \| null }` (required, not optional — `null` means "schedule later"); see "The M9 acceptance transaction" below | **M9** |
| `POST /assignments/:assignmentId/decline` | Decline (recipient only, pre-response) | **M9** |
| `GET /assignments/sent` | Every assignment the authenticated caller has sent (`fromUserId` from the JWT only — no query param accepted), **all** statuses (unlike Inbox), newest first; each row is a `SentAssignmentResponse` (recipient `PublicUser` + the same task-summary shape Inbox uses, again excluding `scheduledAt` — see "Sent response shape" below). No status filter — see REQUIREMENTS.md FR-29. | **M10** |
...
### The M9 acceptance transaction

Acceptance is the one transition that writes two rows (`TaskAssignment` and `Task`) and must never be observed half-applied. The transaction lives in the repository layer, not the service: `taskAssignmentRepository.acceptPendingAssignment` wraps both writes in a single `prisma.$transaction(async (tx) => {...})` interactive transaction — a real Postgres `BEGIN`/`COMMIT`/`ROLLBACK`, not Prisma's `$transaction([...])` batch form, since the second write's `WHERE` clause depends on knowing the first one actually matched a row. Both writes reuse the same atomic-conditional-`updateMany` idiom as `cancelIfPending`/`declineIfPending`/`updateIfNotPending`: each is guarded by a `WHERE` clause tied to the exact state being transitioned *from*. If either affects zero rows, the callback throws a sentinel error, Prisma rolls back, and the repository function returns `null` — nothing partial is ever committed. `AssignmentService.acceptAssignment` stays responsible for everything that isn't part of the atomic write itself: authorization (is the caller `toUserId`?), the pre-check for a clean error in the non-racing case, and validating the recipient's `scheduledAt` choice against the task's `deadline` (EC-13) *before* attempting the transaction — Prisma stays repository/data-access only, per this architecture's layering, so no query logic lives in the service beyond calling the one repository method.

### Concurrency strategy for accept/decline/cancel (EC-3)

Accept, decline (M9), and cancel (M8) all perform their first — and for decline/cancel, only — write on the *same* `TaskAssignment` row via `UPDATE ... WHERE id = ? AND status = 'PENDING'`. This single shared idiom is what makes them mutually exclusive: Postgres takes a row lock on a match, so a second concurrent statement targeting the same row blocks until the first transaction commits or rolls back, then re-evaluates its own `WHERE` clause against the new row state (standard `UPDATE` semantics under `READ COMMITTED`) — since the row's `status` is no longer `'PENDING'`, the second statement matches zero rows and its caller correctly reports `ConflictError`. No advisory locks, semaphores, or `SELECT ... FOR UPDATE` are needed — the guarantee falls directly out of using the identical conditional-`updateMany` pattern everywhere a `PENDING` row can be resolved. See DATABASE.md §8 for the full argument and its empirical verification (double-accept, accept-vs-decline, accept-vs-cancel, decline-vs-cancel, all fired as real concurrent HTTP requests in integration tests).

### Inbox response shape

`InboxAssignmentResponse` is deliberately not "`TaskAssignmentResponse` plus a `TaskResponse`." It's a dedicated, narrower contract (`packages/shared/src/schemas/assignment.ts`) built by a dedicated mapper (`toInboxAssignmentResponse`) because the recipient's Inbox must never see the sender's `scheduledAt` — that field is the *sender's* personal planning state, not a property of the request, and it's about to be replaced or cleared the moment the recipient accepts (FR-30). Excluding it from the schema's type (not just omitting it at the call site) means there's no field left to accidentally leak later. `toUser` is also omitted (always the caller, so redundant), and there's no `nextCursor`/pagination — a personal inbox of `PENDING` requests isn't expected to grow large enough to need it in v1.
...
### Sent response shape (M10)

`SentAssignmentResponse` is the mirror image of `InboxAssignmentResponse`, and deliberately shares its task-summary mapping (`toTaskSummary`, a private helper in `mappers/assignmentResponse.ts` both `toInboxAssignmentResponse` and `toSentAssignmentResponse` call) rather than duplicating the "no scheduledAt" rule in two places. The differences from Inbox are exactly the ones the two lists' purposes demand: `toUser` (the recipient) instead of `fromUser` (always the caller here, so redundant, the same reasoning Inbox already applies in the other direction); `respondedAt` included, since Sent is a historical record where "when was this resolved" is meaningful (Inbox omits it — everything there is still `PENDING`, so it's always `null`); and, at the query level, every `AssignmentStatus` returned, not just `PENDING` — `findSentByUser` has no `status` filter at all, unlike `findInboxForRecipient`.

### Creator-safe task-detail contract (M10)

`GET /tasks/:id`'s authorization split in two, deliberately: **read** access is now assignee-or-creator (`TaskService.findVisibleTaskOrThrow`), but **mutation** access (`PATCH`/`DELETE`) stays assignee-only (`TaskService.findOwnTaskOrThrow`, untouched by M10) — `creatorId` must never substitute for `assigneeId` in a mutation check, so these are two separate functions in `taskService.ts`, not one relaxed check reused everywhere.

The response itself stays a single `TaskDetailResponse` contract (not a second, creator-specific type) but gains a server-computed `viewer: { isAssignee, isCreator, canEdit, canDelete }` object, plus `assignee`/`creator` (`PublicUser`). The client is told, not left to infer, what it may render/do — the alternative (mobile deciding "am I the creator?" by comparing ids itself) would duplicate an authorization decision the server already made, with no guarantee the two ever agree. `scheduledAt` is the one field this contract actively **masks**: for a viewer where `isAssignee` is `false`, the server always sends `scheduledAt: null`, regardless of the task's real stored value — it's the *current assignee's* personal planning state (same rule Inbox/Sent already enforce structurally by omitting the field entirely; here the field exists on the base `TaskResponse` for the assignee case, so it's masked per-viewer instead). `completedAt`, by contrast, is never masked — it describes task execution state, not personal scheduling, so a creator sees it exactly as the assignee does. The mobile client keys its rendering off `viewer.isAssignee`, never off whether `scheduledAt` happens to be `null` — a task can legitimately be unscheduled for an assignee too.
...
| M9 acceptance transaction | A single Prisma interactive `$transaction` in the repository layer (`acceptPendingAssignment`), not two independent service calls (`assignmentService.accept()` + `taskService.changeAssignee()`) | The assignment flip and the `Task.assigneeId`/`scheduledAt` write must never be observed independently; a repository-level transaction is the only place both writes can share one atomic unit without either service reaching into the other's data |
| Accept/decline/cancel concurrency (EC-3) | All three gate their (only, or first) write on the identical `WHERE id = ? AND status = 'PENDING'` condition against the same `TaskAssignment` row | Reuses ordinary Postgres row-level locking to make the three transitions mutually exclusive automatically, with no advisory locks or explicit `SELECT ... FOR UPDATE` needed — see DATABASE.md §8 |
| Where scheduledAt comes from on accept | Only the recipient's request body; the sender's existing `Task.scheduledAt` is never read by the accept path at all | The clearest possible way to guarantee personal planning state can't leak across a responsibility transfer — there's no code path where it even has the opportunity to |
| Inbox response shape | A dedicated `InboxAssignmentResponse`/`InboxTaskSummary` contract, not a reuse of `TaskAssignmentResponse` + `TaskResponse` | `TaskResponse` includes `scheduledAt`, which must never be presented in the recipient's Inbox (see "Inbox response shape" in §3) — a narrower, purpose-built type makes that structurally true rather than a convention to remember |
| Accept/decline/inbox route shape | Flat `/assignments/...`, not nested under `/tasks/:taskId` like create/cancel | These are recipient-scoped (`toUserId`), not sender/assignee-scoped — nesting under a task id the recipient isn't necessarily thinking about would add nothing |
| Read vs. mutation authorization split (M10) | Two separate service functions — `findVisibleTaskOrThrow` (assignee OR creator, read-only callers) and `findOwnTaskOrThrow` (assignee only, mutation callers, unchanged since M3) | Keeps "who may read" and "who may write" as two structurally distinct checks so a future change to one can never accidentally widen the other; `creatorId` must never become a `PATCH`/`DELETE` authorization path |
| Creator-safe scheduledAt masking | Server always overwrites `scheduledAt` to `null` in `GET /tasks/:id`'s response for a non-assignee viewer, computed in `TaskService.getTaskDetail` from the same `viewer.isAssignee` flag the client receives | One response contract, not two; the mask lives exactly where the viewer identity is already known, and the client is told *why* via `viewer` rather than needing to guess from a null value that could otherwise just mean "unscheduled" |
| Sent (M10) reuses Inbox's task-summary mapping | `toTaskSummary` extracted as a private helper in `mappers/assignmentResponse.ts`, called by both `toInboxAssignmentResponse` and `toSentAssignmentResponse` | Both lists need the identical "no scheduledAt" task view for the identical reason; one mapping means that rule can't drift between the two over time |
| Sent status scope | `findSentByUser` has no status filter — returns `PENDING`/`ACCEPTED`/`DECLINED`/`CANCELLED` together, always | Sent is explicitly a *history* view (FR-29), unlike Inbox's PENDING-only actionable queue — filtering it down would recreate Inbox with extra steps |
| Sent mobile surface | An in-screen Incoming/Sent toggle on the existing Inbox tab, not a sixth tab or a new route | Keeps the five-tab bar (Today/Schedule/Tasks/Inbox/Profile) unchanged; incoming and outgoing collaboration are one product area, not two navigation destinations |
| Sent row → task detail (no separate "Sent detail" screen) | Tapping any Sent row navigates to the existing `/tasks/:id` screen | That screen's `viewer`-driven rendering (see "Creator-safe task-detail contract" in §3) already handles every case a Sent row can be in — still-assignee (possibly frozen) or now-creator-only — so a second detail screen would just duplicate that logic without server-driven guarantees |
| Assignment history vs. task deletion (v1 limitation) | Unchanged from M8: `TaskAssignment.task`'s `onDelete: Cascade` still removes assignment history when its task is deleted, including rows a *different* user's Sent list would otherwise show | Documented, accepted v1 tradeoff, not fixed in M10 — preserving history past task deletion needs snapshotting/soft-deletion, a real schema change out of scope for this milestone; see DATABASE.md §8 |
