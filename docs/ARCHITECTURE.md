...
- **Features (`src/features/<feature>/`)** — feature-scoped hooks, components, and API-call wrappers: `src/features/tasks`, `src/features/users`, `src/features/inbox` (M9). TanStack Query hooks live here (`useTodayTasks`, `useCreateTask`, `useUserSearch`, `useInbox`, `useAcceptAssignment`, `useDeclineAssignment`, etc.). **(M8)** Assignment create/cancel hooks (`useCreateAssignment`, `useCancelAssignment`) stayed in `src/features/tasks/useAssignments.ts` rather than a standalone folder, since M8 had no assignment-specific screen yet — the only UI surface was the task detail screen. **(M9)** Now that the Inbox exists as its own screen with its own list/accept/decline concerns, that surface gets its own `src/features/inbox/` folder; the M8 create/cancel hooks stay where they are (still task-detail-scoped, not Inbox-scoped) rather than being merged in, since they serve a different screen and a different caller (the sender, not the recipient).
- **Navigation (Expo Router)** — `app/(app)/_layout.tsx`'s `Tabs` gained a fifth visible destination, `inbox`, alongside Today/Schedule/Tasks/Profile (M9). Its detail/response screen follows the exact pattern already established for Tasks: a sibling folder (`app/(app)/inbox-detail/`) holding a `Stack`-wrapped `[id].tsx`, hidden from the tab bar via `href: null` and reached with `router.push(...)` — never a second `Tabs.Screen`. This was a deliberate reuse of the existing convention rather than a new navigation pattern.
...
| `GET /assignments/sent` | Assignments sent; query: `status` | *(not yet implemented — deferred past M9, see ROADMAP.md)* |
